...
-------------OD TAD POPRAWIC,Dodaæ wstêp przed przyk³adem, rozszerzyæ przyk³ad do n=6, opisac wzor na b³¹d metody trapezów, dodaæ procedurê licz¹ca z wzoru trapezów, porównaæ wyniki otrzymane metod¹ trapezów dla n=6 liczone recznie i dla duzego n liczonego procedur¹, pokazaæ ze im wiêksza liczba podzia³ów tym dok³adniejszy wynik-------- 

\begin{ex}{(Wyznaczanie przybli¿onej wartoœci ca³ki przy wykorzystaniu wzoru trapezów)}\hfill
\textit{Wyznaczyæ przybli¿on¹ wartoœæ ca³ki $\int\limits_0^1 e^{x^2} dx$ przyjmuj¹c, ¿e przedzia³ dzielimy na n=4 równe czêœci(5 wêz³ów)}
...
\end{center}
Wartoœæ dok³adna wynosi 1.46265, zatem ró¿nica w wyniku jest nieznaczna.
\end{ex}
Im wiêcej podzia³ów przedzia³u wykonamy, tym dok³adniejszy otrzymamy wynik. Wynika to bezpoœrednio ze wzoru na b³¹d metody trapezów. Wygl¹da on nastêpuj¹co:
\begin{definition}{(B³¹d przybli¿enia wzorem trapezów)}\\\hfill
\begin{equation}
\mathbf{\delta_T = \frac{(b-a)^3}{12n^2} \cdot M_2},
\end{equation}
gdzie 
\begin{equation}
\mathbf{M_2= \sup_{x \in [a,b]} \, | f^{''}(x) |}
\end{equation}
