...
|BBT_n|\leq\delta_T
\end{equation}
Symbol $\delta_T$ oznacza b³¹d metody trapezów i wyra¿a j¹ nastêpuj¹cy wzór: 
...
Równanie (\ref{wzorSimpsonaBrakPodzialu}) opisuje przypadek, dla którego ca³ka $\int\limits_a^b f(x)dx$ jest przybli¿ana polem znajduj¹cym siê tylko pod jedn¹ parabol¹ na przedziale $[a,b]$. Wykres tej¿e paraboli przechodzi przez punkty $(a=x_0,f(x_0))$, $(\frac{b+a}{2}=x_1,f(x_1))$, $(b=x_2,f(x_2))$. 

Dla przypadku ogólnego, w którym przedzia³ ca³kowania $[a,b]$ zostanie podzielony na $N=2m$ \mbox{($m=1,2,\ldots$)} podprzedzia³ów o równej d³ugoœci $h=\frac{b-a}{N}=\frac{b-a}{2m}$, przy czym ka¿dy wêze³ jest postaci $x_k=a+ih$ \mbox{($i=0,1,2,\ldots, N=2m$)}, to wzór (\ref{wzorSimpsonaBrakPodzialu}) przyjmie nastêpuj¹c¹ formê:
