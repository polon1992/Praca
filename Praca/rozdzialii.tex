...
\begin{figure}[H]
		...
		\end{center}
	   \caption{Graficzne przedstawienie idei zastosowania wzoru trapezÛw do ca≥kowania}
...
\begin{proc}{(Wznaczanie przybliøonych wartoúci ca≥ek pojedynczych metodπ trapezÛw)}\label{MAPLE_MetodaTrapezowCalkaPojedyncza}
\begin{verbatim}
> trapez:=proc(a,b,n) 
...
\begin{proc}{(Wyznaczanie b≥Ídu przybliøenia dla metody trapezÛw)}
...
\end{array}
\end{equation}
RÛwnanie (\ref{wzorSimpsonaZPodzialami}) nosi miano \textit{uogÛlnionego wzoru Simpsona} (mÛwiπc wzÛr \textit{Simpsona} bÍdziemy mieli na myúli tÍ postaÊ uogÛlnionπ). Nie trudno siÍ domyúleÊ, øe w zwiπzku z tym, iø wzÛr ten jedynie przybliøa dok≥adnπ wartoúÊ ca≥ki, to wiπøe siÍ z tym fakt obciπøenia wyniku pewnym b≥Ídem. Oszacowanie owego b≥Ídu przybliøenia wzorem \textit{Simpsona} wyjaúnia poniøsza definicja:
\begin{definition}(B≥πd przybliøenia wzorem \textit{Simpsona})\\
...
|BBS_{N}|\leq\delta_S
\end{equation}
Przez $\delta_S$ rozumiemy b≥πd metody \textit{Simpsona}, ktÛry jest opisany nastÍpujπcym wzorem:
...
Zbadajmy dok≥adnoúÊ, z jakπ wzÛr (\ref{wzorSimpsonaZPodzialami}) przybliøa wartoúci ca≥ek oznaczonych. W tym celu przedstawmy kompletne wyliczenia dla przypadku, gdzie liczba podzia≥Ûw przedzia≥u ca≥kowania bÍdzie wzglÍdnie ma≥a, natomiast sama ca≥ka okreúlona tak jak w przyk≥adzie \ref{PrzykladCalkaPojedynczaMetodaTrapezow}. 

\begin{ex}(Wyznaczanie przybliøonej wartoúci ca≥ki metodπ \textit{Simpsona})\\\hfill\label{PrzykladCalkaLiczonaWzoremSimpsona}
...
Kolejno wstawiamy powyøsze dane do (\ref{wzorSimpsonaZPodzialami}):
\begin{gather*}
...
\begin{proc}{(Wznaczanie przybliøonych wartoúci ca≥ek pojedynczych metodπ \textit{Simpsona})}\label{MAPLE_MetodaSimpsonaCalkaPojedyncza}
\begin{verbatim}
>simpson :=proc(a,b,N) 
...
\begin{proc}{(Wyznaczanie b≥Ídu przybliøenia dla metody \textit{Simpsona})}
...
\end{verbatim}
\end{proc}
Powyøsza procedura ma za zadanie wyznaczyÊ wartoúÊ $\delta_S$, tak by prawdziwe by≥o oszacowanie (\ref{NierownoscBBPrzyblizenia<=DeltaS}). Parametry $a$, $b$ i $N$ definiowane sπ w sposÛb identyczny, jak w \textbf{Procedurze \ref{MAPLE_MetodaSimpsonaCalkaPojedyncza}}. Aby wykona≥ siÍ algorytm g≥Ûwny (znajdujπcy siÍ wewnπtrz procedury w bloku \textit{else}), ponownie musi zostaÊ spe≥niony warunek mÛwiπcy o tym, øe $N$ jest liczπ naturalnπ i parzystπ. Jeøeli taka prawid≥owoúÊ zachodzi, to obliczamy wartoúÊ $f^{(4)}(x)$ oraz upraszczamy jej postaÊ. Zgodnie z (\ref{CzynnikM4DoBleduMetodySimpsona}) wyliczamy maksimum  $f^{(4)}(x)$ na przedziale $[a,b]$ (jest to najbardziej znaczπcy krok) i podstawiamy wszystkie znane juø nam argumenty do (\ref{BladMetodySimpsona}). W ten sposÛb dochodzimy do poøπdanej przez nas formy $\delta_S$. 

Posiadajπc juø odpowiednie narzÍdzia (Procedura \ref{MAPLE_MetodaSimpsonaCalkaPojedyncza} i \ref{MAPLE_BladMetodySimpsonaCalkaPojedyncza}) oraz wiedzÍ z zakresu metodyki ich dzia≥ania, wykonajmy pomiary, ktÛre zobrazujπ nam w sposÛb praktyczny dok≥adnoúÊ metody \textit{Simpsona}. Niech $N$ stanowi liczbÍ podzia≥Ûw przedzia≥y ca≥kowania, $W_{simpson}$ opisuje wartoúci zwracane przez procedurÍ \ref{MAPLE_MetodaSimpsonaCalkaPojedyncza}. Ponadto  przez $|BBS_{N}|$ oznaczymy b≥πd bezwzglÍdnym przybliøenia wartoúci ca≥ki metodπ \textit{Simpsona}, natomiast przez symbol $\delta_S$ rozumieÊ bÍdziemy wynik wywo≥ania \textbf{Procedury \ref{MAPLE_BladMetodySimpsonaCalkaPojedyncza}}. Do obliczeÒ ponownie wykorzystajmy ca≥kÍ z przyk≥adu \ref{PrzykladCalkaLiczonaWzoremSimpsona} (zastosujemy rÛwnieø jej wartoúÊ dok≥adnπ, ktÛra jest podana w jego rozwiπzaniu):
