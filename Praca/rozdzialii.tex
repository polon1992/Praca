...
I(f)=\int\limits_a^b f(x) dx \approx \int\limits_a^b L_n(x) dx=
\sum\limits_{i=0}^n a_i f(x_i) dx= S(f),
\end{equation}
gdzie 
...
W wyprowadzonym wzorze trapezów (\ref{OgolnyWzorTrapezow}) (jak sama nazwa sugeruje) przybli¿amy wartoœæ ca³ki sumuj¹c pola trapezów o ustalonej wysokoœci $h$ (odleg³oœæ pomiêdzy kolejnymi wêz³ami) i podstawach o d³ugoœci $f(x_i)$ i $f(x_{i+1})$ dla $i=0,1,\ldots,n-1$. Rysunek \ref{R2} z pewnoœci¹ pomo¿e nam lepiej zrozumieæ zagadnienie, na którym siê skupiamy.
\begin{figure}[H]
		\begin{center}
...
PrzejdŸmy do praktycznego zastosowania wzoru (\ref{OgolnyWzorTrapezow}) w celu pokazania jego przydatnoœci. Obliczenia dla zadanej funkcji przeprowadzimy rêcznie - przy relatywnie ma³ej liczbie podzia³u przedzia³ów, oraz wykorzystuj¹c procedurê stworzon¹ w jêzyku \textit{Maple} dla du¿o wiêkszej liczby podzia³ów. 
...
\begin{figure}[H]
		\begin{center}
		\includegraphics[scale=0.73]{R3.eps} 
		\end{center}
...
Podstawiaj¹c obliczone powy¿ej wartoœci $a_0, a_1, a_2$ do (\ref{WzorKwadratura}) otrzymujemy szukany przez nas wzór \textit{Simpsona}: 
...
\end{array}
\end{equation}
Zauwa¿my, ¿e dla tego przypadku zachodzi równie¿ prawid³owoœæ zwi¹zana z wartoœci¹ sumy wspó³czynników, adekwatna do tej ze wzoru trapezów: $a_0+a_1+a_2=\frac{h}{2}+\frac{4h}{3}+\frac{h}{3}=n\cdot h=2\cdot h$. Mo¿emy zatem wnioskowaæ, ¿e powy¿sze obliczenia zosta³y przeprowadzone prawid³owo.
...
\end{array}
\end{equation}
Równanie (\ref{wzorSimpsonaZPodzialami}) nosi miano \textit{uogólnionego wzoru Simpsona}. Nie trudno siê domyœleæ, ¿e w zwi¹zku z tym, i¿ wzór ten jedynie przybli¿a dok³adn¹ wartoœæ ca³ki, to wi¹¿e siê z tym fakt obci¹¿enia wyniku pewnym b³êdem. Oszacowanie owego b³êdu przybli¿enia dla wzoru \textit{Simpsona} wyjaœnia poni¿sza definicja:
...
M_4= \sup\limits_{x \in [a,b]} \, | f^{(IV)}(x) |.
\end{equation}
\end{definition} 
...
Kolejno wstawiamy wyznaczone wartoœci do (\ref{wzorSimpsonaZPodzialami}):
\begin{gather*}
...
\end{gather*}
\end{ex}
Wynik, który otrzymaliœmy w przeprowadzonych powy¿ej obliczeniach jest bardzo zbli¿ony do wartoœci dok³adnej $\int\limits_0^{\frac{3}{2}} e^{x^2} dx=4,06311405862...$. Chcemy jednak sprawdziæ, przy jak du¿ej liczbie podzia³ów przedzia³u ca³kowania przybli¿enie zacznie siê stabilizowaæ tak, byœmy mogli uznaæ je za wystarczaj¹co dok³adne (przyjmijmy, ¿e b³¹d bezwzglêdny przybli¿enia ma byæ nie wiêkszy ni¿ $0,0000001$). Podobnie jak dla metody trapezów, wykorzystajmy stworzone na nasze potrzeby procedury automatyzuj¹ce przeprowadzanie obliczeñ. \\ 
...
Powy¿szy skrypt tworzy procedurê \textit{Maple}, realizuj¹c¹ algorytm wyznaczania przybli¿onej wartoœæ ca³ki pojedynczej metod¹ \textit{Simpsona}. Jej parametrami wejœciowymi s¹:\\
$\mathbf{a}$ - wartoœæ dolnej granicy ca³kowania\\
$\mathbf{b}$ - wartoœæ górnej granicy ca³kowania\\
$\mathbf{N}$ - opisuje iloœæ podzia³ów przedzia³u ca³kowania funkcji $f(x)$ (konieczne jest zdefiniowanie $f(x)$ przed wywo³aniem procedury). Wartoœæ parametru $N$ musi byæ liczb¹ naturaln¹ i podzieln¹ przez $2$, gdy¿ zastosowanie metody parabol  wymaga co najmniej $2$ podprzedzia³ów wydzielonych przez $3$ równo odleg³e wêz³y.\\
...
\end{verbatim}
\end{proc}
Powy¿sza procedura ma za zadanie wyznaczyæ wartoœæ $\delta_S$, tak by prawdziwe by³o oszacowanie (\ref{NierownoscBBPrzyblizenia<=DeltaS}). Parametry $a$, $b$ i $N$ definiowane s¹ w sposób identyczny, jak w procedurze \ref{MAPLE_MetodaSimpsonaCalkaPojedyncza}. Aby wykona³ siê algorytm g³ówny (znajduj¹cy siê wewn¹trz procedury w bloku \textit{else}), ponownie musi zostaæ spe³niony warunek mówi¹cy o tym, ¿e $N$ jest licz¹ naturaln¹ i parzyst¹. Je¿eli taka prawid³owoœæ zachodzi, to obliczamy wartoœæ $f^{(IV)}(x)$ oraz upraszczamy jej postaæ. Zgodnie z (\ref{CzynnikM4DoBleduMetodySimpsona}) wyliczamy maksimum  $f^{(IV)}(x)$ na przedziale $[a,b]$ (jest to najbardziej znacz¹cy krok) i podstawiamy wszystkie znane ju¿ nam argumenty do (\ref{BladMetodySimpsona}). W ten sposób dochodzimy do po¿¹danej przez nas formy $\delta_S$. 

Posiadaj¹c ju¿ odpowiednie narzêdzia (procedury \ref{MAPLE_MetodaSimpsonaCalkaPojedyncza} i \ref{MAPLE_BladMetodySimpsonaCalkaPojedyncza}) oraz wiedzê z zakresu metodyki ich dzia³ania, wykonajmy pomiary, które zobrazuj¹ nam w sposób praktyczny dok³adnoœæ metody \textit{Simpsona}. Niech $N$ bêdzie liczb¹ podzia³ów przedzia³y ca³kowania, $W_{simpson}$ opisuje wartoœci zwracane przez procedurê \ref{MAPLE_MetodaSimpsonaCalkaPojedyncza}. Ponadto $|BBS_{N}|$ bêdzie oznacza³o b³¹d bezwzglêdnym przybli¿enia wartoœci ca³ki metod¹ \textit{Simpsona}. Przez symbol $\delta_S$ rozumieæ bêdziemy wynik wywo³ania procedury \ref{MAPLE_BladMetodySimpsonaCalkaPojedyncza}. Rozpatrzmy ca³kê z przyk³adu \ref{PrzykladCalkaLiczonaWzoremSimpsona} (wykorzystamy jej wartoœæ dok³adna - jest ona podana w rozwi¹zaniu), wtedy:
...
\hline 
 ...
 \hline
 40  & 4,06311873247 & 0,00007399099 & 0,00003142697\\ 
 \hline 
...
Analizuj¹c wyniki z  powy¿szej tabeli mo¿na bardzo szybko zauwa¿yæ, ¿e metoda \textit{parabol} jest bardziej optymalna ni¿ metoda \textit{trapezów}. B³¹d bezwzglêdny przybli¿enia spad³ poni¿ej $0,0000001$ ju¿ dla przypadku, w którym dokonaliœmy $N=106$ podzia³ów przedzia³u ca³kowania, dla porównania w metodzie trapezów podzia³ów tych musieliœmy wykonaæ a¿ $n=7310$. 
