...
\begin{ex}{(Wyznaczanie przybli¿onej wartoœci ca³ki przy wykorzystaniu wzoru trapezów)}\hfill
...
$=\frac{1}{8}(e^0+2e^{\frac{1}{4}}+2e^{\frac{2}{4}}+2e^{\frac{3}{4}}+2e^1+2e^\frac{5}{4}+e^{\frac{6}{4}})\approx \frac{1}{8}(1+2\cdot 1,28403+2\cdot 1,64872+2\cdot 2,11700+2\cdot 2,71828+2\cdot 3,49034+4,48168)\approx 3,4998$\\
\end{center}
Wartoœæ dok³adna wynosi w zaokr¹gleniu 4,063114. Jak widaæ ró¿nica w wyniku jest dosyæ znacz¹ca przy ustalonych wartoœciach wêz³ów!
\end{ex}


Im wiêcej podzia³ów przedzia³u wykonamy, tym dok³adniejsze otrzymamy przybli¿enie. Wynika to bezpoœrednio ze wzoru na b³¹d metody trapezów:
...
\end{definition}
Sprawdzimy jednak s³usznoœæ naszego twierdzenia wykorzystuj¹c do tego stworzon¹ na nasze potrzeby procedurê w jêzyku Maple. Pos³u¿y nam ona do obliczania przybli¿onych wartoœci ca³ek oznaczonych metod¹ trapezów:

\begin{proc}{(Procedura obliczaj¹ca wartoœci ca³ek metod¹ trapezów)}
\begin{verbatim}
> trapez:=proc(a,b,N) 
...
\end{verbatim}
\end{proc}
Przeanalizujmy wyniki otrzymane nastêpuj¹c¹ procedur¹ dla $n=6$ podzia³ów oraz $n=100$ podzia³ów

%-------------DODAC: opis procedury na wzor trapezow, porównaæ wyniki otrzymane metod¹ trapezów dla n=6 liczone recznie i dla duzego n liczonego procedur¹, pokazaæ ze im wiêksza liczba podzia³ów tym dok³adniejszy wynik, pokazaæ ze mozna wyliczyæ minimaln¹ liczbê podzia³u przedzia³ów by otrzymaæ wynik z maksymalnie okreslonym bledem-------- 
