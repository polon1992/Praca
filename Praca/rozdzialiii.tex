\chapter{Kwadratury Newtona-Cotesa dla ca³ek podwójnych w prostok¹cie}
W ca³ym poprzednim rozdziale skupialiœmy siê na metodach ca³kowania numerycznego. Odnosi³y siê one jednak tylko do funkcji jednej zmiennej - zatem ca³kowanie odbywa³o siê na pewnym przedziale domkniêtym. Teraz chcemy poszerzyæ nasz¹ wiedzê i przejœæ o jeden krok dalej. Nasze rozwa¿ania zostan¹ skierowane ponownie na te same, co poprzednio metody ca³kowania numerycznego. Znacz¹ca ró¿nica polega³a bêdzie na tym, ¿e zajmiemy siê ca³kowaniem funkcji dwóch zmiennych. Wszystkie nasze dzia³ania maj¹ na celu wy³onienie najdok³adniejszej metody wyznaczania wartoœci ca³ek podwójnych przy pomocy prezentowanych algorytmów.  W badaniach wyró¿nimy trzy typy obszarów ca³kowania, bêd¹ to:
\begin{itemize}
\item[a)] Obszar normalny ze wzglêdu na zmienn¹ $x$ i $y$
\item[b)] Obszar normalny tylko ze wzglêdu na zmienn¹ $x$
\item[c)] Obszar normalny tylko ze wzglêdu na zmienn¹ $y$
\end{itemize}

W celu kontynuacji rozwa¿añ musimy przypomnieæ niezbêdn¹ dla nas definicjê normalnoœci obszaru:
\begin{definition}(Obszar normalny)\\
O obszarze normalnym w przestrzeni kartezjañskiej $\re ^2$ 
\begin{itemize}
\item[a)]\label{ObszaryNormalneOx}wzglêdem osi $OX$ (tj. wzglêdem $x$) mówimy, gdy podzbiór p³aszczyzny kartezjañskiej ograniczony jest przez wykresy pewnych dwóch funkcji ci¹g³ych $u(x)$ i $v(x)$ oraz dwie proste równoleg³e do osi $OY$.    
\item[b)]\label{ObszaryNormalneOy}wzglêdem osi $OY$ (tj. wzglêdem $y$) mówimy, gdy podzbiór p³aszczyzny kartezjañskiej jest wyznaczany poprzez wykresy dwóch ró¿nych funkcji ci¹g³ych $l(y)$ i $k(y)$ oraz dwie proste, które s¹ równoleg³e do osi $OX$.
\item[c)]ze wzglêdu na zmienne $x$ i $y$ mówimy, gdy podzbiór p³aszczyzny kartezjañskiej jest ograniczony przez dwie proste równoleg³e do osi $OX$ oraz dwie proste \mbox{równoleg³e do $OY$} 

\end{itemize}

\section{Wzór trapezów }\label{Rozdzial3.1_WzorTrapezow2DObszNormalny}
...
\end{itemize}
Otrzymane wyniki zaprezentujemy poni¿ej w formie tabel. Przyjmijmy, ¿e $n$ oznacza³o bêdzie iloœæ podzia³ów przedzia³u ca³kowania dla zmiennej $x$, $m$ dla zmiennej $y$, zaœ $W_{trapez2}$ to wynik \textit{Trapez2} dla coraz wiêkszych wartoœci $n$ i $m$. Ponadto niech \textit{$BBT2_{n,m}$} opisuje b³¹d bezwzglêdny przybli¿enia wartoœci ca³ki podwójnej procedur¹ \textit{Trapez2}, przy jej wywo³aniu dla pewnych parametrów $n$ i $m$. Dodatkowo \textit{$BWT2_{n,m}$} to wielkoœæ b³êdu wzglêdnego jakim jest obci¹¿ony ten sam wynik wykonania \textit{Trapez2}, co w przypadku wyliczania \textit{$BBT2_{n,m}$}.
...
Wartoœæ dok³adna ca³ki z przyk³adu a) jest równa $0,399181467986$ , wiêc:. 
...
Przedzia³ $[a,b]$ zostanie podzielony na $N=2n$, $(n=1,2,\ldots)$ podprzedzia³ów postaci \mbox{$[x_{2i},x_{2i+1}] (i=0,\frac{1}{2},\ldots, \frac{2n-1}{2})$}. Podobnie bêdzie dla $[c,d]$, podzielimy go na $M=2m$, $(m=1,2,\ldots)$ podprzedzia³ów \mbox{$[y_{2j},y_{2j+1}]$ ($j=0,\frac{1}{2},\ldots, \frac{2m-1}{2}$).} Z tego faktu wynika³a bêdzie nastêpuj¹ca prawid³owoœæ:
...
\end{array}
\end{equation}
W celu wyeliminowania z równania (\ref{WzorSimpsona2JednaCalka}) koniecznoœci ca³kowania, mo¿emy ponownie zastosowaæ uogólniony wzór \textit{Simpsona} - tak jak zrobiliœmy to dla przypadku (\ref{Funkcja_g(x)_CalkaPodwojnaMetodaSimpsona}).
\newpage
...
Tak jak zauwa¿yliœmy wczeœniej dla uogólnionego wzoru trapezów (\ref{UogolnionyWzorTrapezow2_obszarNormalny}), rozpisanie  (\ref{UogolnionyWzorSimpsona2_obszarNormalny}) w sposób taki, by uwzglêdniono w nim wszystkie mo¿liwe punkty wêz³owe postaci $(x_{2i},y_{2j})$ ( $i=0,1,\ldots, n$\,\,,\, \mbox{$j=0,1,\ldots, m$} ) nale¿¹ce do pewnego obszaru prostok¹tnego oraz spe³niaj¹ce równanie (\ref{WezlyNaObszarzeProstokatnymSimpson2}), skutkowa³oby tym, ¿e wartoœci przy kolejnych $f(x_{2i},y_{2j})$ ($i=0,1,\ldots,n\,\,,\,\,\,\, j=0,1,\ldots,m$ ) rozk³ada³yby siê wed³ug pewnego schematu powsta³ego w wyniku mno¿enia dwóch nastêpuj¹cych wektorów:
...
Zaprezentowana macierz diametralnie upraszcza zapis \textbf{uogólnionego wzoru Simpsona dla ca³ek podwójnych po obszarach normalnych}. Pewien jej fragment, a dok³adniej pierwszy wiersz, pojawi³ siê ju¿ dla przypadku uogólnionego wzoru Simpsona (\ref{wzorSimpsonaZPodzialami}) dla ca³ki pojedynczej.
...
Ka¿dy z podobszarów obszaru $\Omega$ jest ograniczonych pogrubion¹ krawêdzi¹ i sk³ada siê zawsze z dok³adnie czterech mniejszych podpodobszarów wyznaczanych przez dziewiêæ wêz³ów ($x_{2i},y_{2j}$) ( $i=0,1,\ldots, n$\,\,,\, \mbox{$j=0,1,\ldots, m$} ). Objêtoœæ dowolnego z tych wiêkszych wyra¿a wzór:
...
Rozszerzaj¹c zakres sumowania z jednego podobszaru na ca³¹  $\Omega$ nasz wzór przyjmie ostatecznie bardziej czyteln¹ formê:
...
Zaprezentowane dotychczas sposoby wyprowadzania \textit{uogólnionego wzoru Simpsona} nie s¹ jedynymi nam znanymi. Poka¿my, ¿e (zgodnie ze stwierdzeniem z pocz¹tku rozdzia³u) to samo równanie mo¿emy otrzymaæ poprzez ca³kowanie wielomianu \textit{Lagrange'a} $L_{2,2}(x,y)$. Ca³ka taka jest postaci:
...
\end{equation}       
Przyjmuj¹c nastêpuj¹ce wartoœci $x_0=0$, $x_1=h$, $x_2=2h$, $y_0=0$, $y_1=k$ oraz $y_2=2k$ mo¿emy w (\ref{WzorSimpsona2DZCalkiL22}) wyszczególniæ dziewiêæ ca³ek. Wyznaczenie ich spowoduje otrzymanie wspó³czynników wystêpuj¹cych przy wartoœciach funkcji $f(x_i,y_j)$ w kolejnych wêz³ach zgodnych z (\ref{MacierzWspolczynnikowSimpson2_objetosc1podobszaru}), tzn.:
...
\end{center}
%W tym momencie uwidacznia siê zdecydowanie wiêksza dok³adnoœæ \textit{uogólnionego wzoru Simpsona} w zestawieniu go z \textit{uogólnionym wzorem Trapezów}. 
Dla \textit{wzoru trapezów }wielkoœæ BW (dla takiej samej ca³ki) przy wykonanych dwóch podzia³ach przedzia³u ca³kowania oscylowa³a w granicach $0,2\%$, wiêc by³a ona niemal 15 krotnie wiêksza!
\end{ex}
Algorytm \textit{Simpsona} (\ref{SumaObjetosciDlaCalegoObszaruSimpson2D}) pomimo tego, ¿e na pierwszy rzut oka wydaje siê byæ skuteczniejszy ni¿  wspomniany \textit{wzór Trapezów} (\ref{RownoscUogWzoruTrapezow2IWzoruZObjetosciami}), to jest równie¿ du¿o bardziej skomplikowany je¿eli chodzi o z³o¿onoœæ rachunków jakie nale¿y wykonaæ. Ka¿de zwiêkszenie liczby wêz³ów powoduje bardzo szybki rozrost iloœci wartoœci funkcji do wyznaczenia. W zwi¹zku z tym, by kontynuowaæ rozwa¿ania zaprezentujmy stworzon¹ procedurê automatyzuj¹c¹ obliczanie wartoœci ca³ek podwójnych po obszarach normalnych \textit{uogólnionym wzorem Simpsona}.
...
Pierwsza linia powy¿szego kodu jest deklaracj¹ nazwy procedury oraz jej parametrów wejœciowych, przy czym:\\
a, b -  dolna i górna granica ca³kowania wzglêdem zmiennej $x$,\\
c, d -  dolna i górna granica ca³kowania wzglêdem zmiennej $y$,\\
N - liczba podpodprzedzia³ów na jakie zostanie podzielony ca³y przedzia³ ca³kowania $[a,b]$,\\
M - liczba podpodprzedzia³ów na jakie zostanie podzielony ca³y przedzia³ ca³kowania $[c,d]$.\\
Kolejno wymienione s¹ wszystkie zmienne lokalne, jakie bêd¹ wykorzystane w obliczeniach wykonywanych wewn¹trz algorytmu. Pojawiaj¹ca siê nastêpnie instrukcja warunkowa \textit{if-else} decyduje o tym, czy parametry wywo³ania zosta³y podane prawid³owo. Je¿eli $N$ lub $M$ nie jest liczb¹ naturaln¹ parzyst¹ to zwracany jest stosowny komunikat o b³êdzie. W przeciwnym przypadku rozpoczyna siê wykonanie w³aœciwej czêœci algorytmu, w sk³ad którego wchodzi okreœlenie iloœci podprzedzia³ów $n$, $m$ (ka¿dy podprzedzia³ z³o¿ony jest z dwóch podpodprzedzia³ów) na które podzielono przedzia³y ca³kowania po zmiennej $x$ i $y$. Dalej na podstawie (\ref{WezlyNaObszarzeProstokatnymSimpson2}) wyznaczane s¹ wielkoœci kroków $h$ (dla $x$) i $k$ (dla $y$) pomiêdzy kolejnymi wêz³ami definiuj¹cymi krañce owych podpodprzedzia³ów. Pozyskane dane zostaj¹ natychmiast wykorzystywane przez pêtle \textit{for} do obliczenia wspó³rzêdnych wêz³ów $x_i$ oraz $y_j$. Blok z dwoma zagnie¿d¿onymi pêtlami \textit{for} s³u¿y wyliczaniu wartoœci funkcji $z(i,j)=f(x_i,y_j)$ w okreœlonych (w poprzednich dwóch krokach) punktach wêz³owych. Przedostatni krok jest implementacj¹ wzoru (\ref{SumaObjetosciDlaCalegoObszaruSimpson2D}), co czyni go kluczowym dla ca³ego algorytmu. Ostatecznie wynik procedury jest zwracany w uproszczonej i czytelnej postaci jako wartoœæ \textit{WynikS2}.

Wiedz¹c ju¿ na jakiej zasadzie dzia³a procedura \textit{Simpson2} - u¿yjmy jej do przeprowadzenia badañ nad dok³adnoœci¹ pozyskanych dziêki niej przybli¿onych wyników ca³kowania. Rezultaty porównamy z tymi, otrzymanymi przez procedurê \ref{MAPLE_Trapez2ObszarNormalnyWartoscCalki} (\textit{Trapez2}). Przeliczmy zatem nastêpuj¹ce ca³ki: 
\begin{itemize}
\item[a)] $\int\limits_0^\frac{1}{2}\int\limits_\frac{1}{2}^1 \frac{x+y}{x^2+y^2} dydx$
\item[b)] $\int\limits_0^1\int\limits_1^2 e^\frac{x^2}{y^3} dydx$
\end{itemize}
Tak jak w  podrozdziale poprzednim wyniki przedstawimy w formie tabel. Niech $N$ oznacza liczbê podpodprzedzia³ów na które podzielono przedzia³ ca³kowania po zmiennej $x$, $M$ po zmiennej $y$, $W_{Simpson2}$ to wynik \textit{Simpson2} dla rosn¹cych wartoœci $N$ i $M$. Niech \textit{$BBS2_{N,M}$} bêdzie b³êdem bezwzglêdnym przybli¿enia wartoœci ca³ki podwójnej procedur¹ \textit{Simpson2} dla pewnych parametrów $N$ i $M$. Dodatkowo \textit{$BWS2_{N,M}$} to wielkoœæ b³êdu wzglêdnego jakim jest obci¹¿ony ten sam wynik wykonania \textit{Simpson2}, co w przypadku wyliczania \textit{$BBS2_{N,M}$}.\\

Wartoœæ ca³ki z przyk³adu a) jest w zaokr¹gleniu równa $0,399181467986$, wiêc:
\begin{table}[h!]
\centering
 \begin{tabular}{|c|c|c|c|c|} 
 \hline
 ...
 \hline 
 \end{tabular}\\ \hfill
 \caption{Pomiary dok³adnoœci przybli¿ania wartoœci ca³ki a) \textit{uogólnionym wzorem Simpsona}}
...
\centering
 ...
 \hline 
 \end{tabular}\\ \hfill
 \caption{Pomiary dok³adnoœci przybli¿ania wartoœci ca³ki b) \textit{uogólnionym wzorem Simpsona}}
 \label{WynikiZProcedurySimpson2b}
\end{table}


Dokonuj¹c przegl¹du i analizy wyników z Tablicy \ref{WynikiZProcedurySimpson2a} i Tablicy \ref{WynikiZProcedurySimpson2b} pierwsze co powinno rzuciæ siê nam w oczy to wyj¹tkowa dok³adnoœæ otrzymanych wartoœci. Wielkoœæ b³êdu bezwzglêdnego ju¿ dla pierwszego wywo³ania procedury w obydwu przyk³adach nie przekroczy³a $0.0031$, jest to bardzo dobry rezultat. Dodatkowo po ka¿dym zwiêkszeniu parametrów $N$ lub $M$ widoczna jest stanowcza tendencja spadkowa \textbf{$BBS2_{N,M}$} jak i \textbf{$BWS2_{N,M}$}. Dla koñcowych pomiarów mo¿na powiedzieæ, ¿e wynik jest niemal nie obci¹¿ony b³êdem przybli¿enia, poniewa¿ jego wielkoœæ jest bliska $0$ dla obydwu przyk³adów. Interesuj¹ce jest to, ¿e pomiêdzy b³êdami, którymi obci¹¿one s¹ wyniki otrzymane \textit{uogólnionym wzorem trapezów} a \textit{uogólnionym wzorem Simpsona} jest œrednio dwa rzêdy wielkoœci ró¿nicy (na korzyœæ wzoru \textit{Simpsona}). Fakt ten sprawia, ¿e mo¿e on zostaæ uznany za dok³adniejszy i zarazem bardziej efektywny.    
